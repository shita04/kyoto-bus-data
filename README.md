...
**`max_occ = 0` は2つの意味を持つ。`n_occ` を見て区別すること。**

| `n_occ` | `max_occ` | 意味 |
|---|---|---|
| 1 以上 | 0 | **空車。** 実際に `EMPTY`（0）が観測された |
| 0 | 0 | **データなし。** 混雑度が1件も取れなかった（`sum_occ` も 0 になる） |

`n_occ = 0` の行で `sum_occ / n_occ` を計算するとゼロ除算になる。また `max_occ` を
そのまま「空いている」と解釈すると、データが無いだけの停留所を「常に空車」と
誤って扱ってしまう。**混雑度を使う集計では、まず `n_occ > 0` の行に絞り込むこと。**

なお `n_occ = 0` の行は 2026-09-07 の修正で初めて現れる。それ以前は
`occupancy_status = 7`（データなし）も混雑度として数えていたため、
`n_occ` は常に `n_obs` と同じ値だった。
